...
},
{
  ...
  "body": " Bases and the Way We Grid Space   In this section we look at how we can grid space in many different ways and how we can move between these gridding systems. Recall from , we used two different gridding systems to describe the location of a fly on the ceiling.     Carter Caves State Park: Revisited   Descartes Fly     Descartes' Fly: Revisited   Descartes Fly      You will also recall that in we explored how many vectors we needed for linear combinations of these vectors to describe a location in . In particular, we found that if we had three or more vectors that were linearly indepndent in pairs, we did not need them all. We only needed two linearly independent vectors to be able to describe any location in with a linear combination of them.    Bases: Minimal Spanning Sets  At this point, we want to explore how different ways of gridding a space can have different advantages and disadvantages. From , we had seen each of two grid systems as well as these superimposed on each other.      Fly and Grid      Fly and Grid      Fly and Both Grids      In the \"standard\" or Cartesian grid system shown in , the linear combination of vectors to reach the fly was . While the linear combination of vectors from the grid system was . Assuming that each of these systems give unique linear combinations of their respective vectors to reach any vector in , we could represent the fly's location in two different ways. In the \"standard\" set of vectors, , the coordinates would be . On the other hand, using the set, , the coordinates would be . Note that if we listed the vectors in different orders in the set of vectors, say or , we would need to reverse the order the coordinates are given in the ordered pairs . After all, they are called ordered pairs .    Invertible Matrices and Moving Between Grid Systems  So one question we might ask is why would we care which system we use? At times, the ease of transforming vectors in one system may be less complicated than another. This means that it might be desirable at times to convert to an easier grid system. So how do we make the conversion back and forth between different systems?   "
},
{
